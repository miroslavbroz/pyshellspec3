        ...
        if hdu.name == 'OI_VIS':
            for row in data:
                if header['DATE-OBS'].find('-') >= 0:
                    ...
                    date = header['DATE-OBS'].split('/')
                    timeobs = datetime.datetime(int(date[2]), int(date[1]), int(date[0])) + datetime.timedelta(seconds=np.around(row.field('TIME'), 2))
            ...
            for row in data:
                ...
                # A workaround for VEGA data --- there seem to be this DATE-OBS in wrong format
                if len(date[0]) == len(header['DATE-OBS']):
                    date = header['DATE-OBS'].split('/')
                    if int(date[-1]) > int(date[0]):
                        temp = date[0]
                        date[0] = date[-1]
                        date[-1] = temp
                    if len(date[0]) == len(header['DATE-OBS']):
                        raise ValueError('I do not understand the format of DATE-OBS!')
                         
                timeobs = datetime.datetime(int(date[0]), int(date[1]), int(date[2])) + datetime.timedelta(seconds=np.around(row.field('TIME'), 4))
                int_time = row.field('INT_TIME')
