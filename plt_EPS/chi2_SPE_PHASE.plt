...
p \
  ...
  "<sort -k8 -g *.spe.syn.dat | awk '($7!=last){ i++; print $0,i; }{ last=$7; }'" u ($2/nm):($6+$10*fac-2*fac+0.30):(sprintf("{/=12 %.3f }",$8)) not w labels right,\
  "<sort -k8 -g *.spe.syn.dat | awk '($7!=last){ i++; print $0,i; }{ last=$7; }'" u ($2/nm):($6+$10*fac-2*fac-0.30):(sprintf("{/=12 %.0f }",$7)) not w labels right,\
